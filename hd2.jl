#error masa 1.5%, energia 67%, concentracion verticalmente uniforme, tarda 5 horas
...
mu_a, rho_a, cp_a, ka, Da = 1.8e-5, 1.15, 1006.0, 0.026, 2.6e-5
mu_d, rho_d, kd, g = 0.005, 1200.0, 0.61, 9.81
h_fg = 2450000.0

T_air_in, W_air_in = 308.15, 0.025
T_liq_in, xi_liq_in = 293.15, 0.40

# --- 2. PERFILES DE VELOCIDAD ANALÍTICOS ---
...
println("Iniciando iteraciones...")
println("Iter\t| Q_air (W)\t| Q_liq (W)\t| Error E (%)")
println("-"^50)

@time for iter in 1:3000
    Ta_old, Wa_old, Td_old = copy(Ta), copy(Wa), copy(Td)
    
    # --- AIRE: Propiedades variables ---
# AIRE (i: 1 -> Nx)
    for i in 1:Nx
        for j in 1:Ny
            ...
            curr_k   = _kₐ(Ta[i,j], Wa[i,j])
            curr_D   = _Da(Ta[i,j]) 

            if j == 1 # INTERFAZ (Contacto con el líquido)
                pv_s = _Pᵥₐₚₒᵣ_ₛₒₗ(Td[i, Ny], xi_d[i, Ny], LiCl())
                W_int = 0.622 * pv_s / (101325.0 - pv_s)
                
                # Factor 2 por media celda (distancia dya/2)
                Wa[i,j] = ( (u_a[j]/dx)*Win + (curr_D/dya^2)*Wa[i,j+1] + (2*curr_D/dya^2)*W_int ) / 
                        ( (u_a[j]/dx) + (curr_D/dya^2) + (2*curr_D/dya^2) )
                
                Ta[i,j] = ( (u_a[j]/dx)*Tin + (curr_k/(curr_rho*curr_cp*dya^2))*Ta[i,j+1] + (2*curr_k/(curr_rho*curr_cp*dya^2))*Td[i, Ny] ) / 
                        ( (u_a[j]/dx) + (curr_k/(curr_rho*curr_cp*dya^2)) + (2*curr_k/(curr_rho*curr_cp*dya^2)) )

            elseif j == Ny # PARED ADIABÁTICA (Límite superior)
                # Aplicamos condición de Neumann: W[j+1] = W[j-1]
                Wa[i,j] = ( (u_a[j]/dx)*Win + (curr_D/dya^2)*(2*Wa[i,j-1]) ) / 
                        ( (u_a[j]/dx) + 2*curr_D/dya^2 )
                
                Ta[i,j] = ( (u_a[j]/dx)*Tin + (curr_k/(curr_rho*curr_cp*dya^2))*(2*Ta[i,j-1]) ) / 
                        ( (u_a[j]/dx) + 2*curr_k/(curr_rho*curr_cp*dya^2) )

            else # NODOS INTERNOS
                Wa[i,j] = ( (u_a[j]/dx)*Win + (curr_D/dya^2)*(Wa[i,j+1] + Wa[i,j-1]) ) / 
                        ( (u_a[j]/dx) + 2*curr_D/dya^2 )
                
                Ta[i,j] = ( (u_a[j]/dx)*Tin + (curr_k/(curr_rho*curr_cp*dya^2))*(Ta[i,j+1] + Ta[i,j-1]) ) / 
                        ( (u_a[j]/dx) + 2*curr_k/(curr_rho*curr_cp*dya^2) )
            end
        end
    end
    # --- LÍQUIDO: Balance con Calor de Mezcla Variable ---
    for i in Nx:-1:1
        # Propiedades de la interfaz
        T_int = Td[i, Ny]
        xi_int = xi_d[i, Ny]
        
        pv_s = _Pᵥₐₚₒᵣ_ₛₒₗ(T_int, xi_int, LiCl())
        W_sat = 0.622 * pv_s / (101325.0 - pv_s)
        
        # Jm: Flujo de masa real (kg/m²s)
        jm = (_ρₐ(Ta[i,1], Wa[i,1]) * _Da(Ta[i,1]) / (dya/2)) * (Wa[i, 1] - W_sat)
        ...
        for j in 1:Ny
            ...
            if j == Ny
                # q_sens: Calor desde el aire
                q_sens = _kₐ(Ta[i,1], Wa[i,1]) * (Ta[i, 1] - T_int) / (dya/2)
                
                # q_lat: Calor latente + Calor de mezcla (exotérmico)
                # Usamos tus nuevas funciones i_fg(T) y _Δh(T, xi)
                q_lat = jm * (i_fg(T_int) + _Δh(T_int, xi_int, LiCl()))
                
                Tn = T_int + (q_sens + q_lat) * (dyd / λ_H2O(T_int)) 
            else
                Tn = Td[i, j+1]
            end

            # cp_sol depende del nuevo cp_H2O(T)
            cp_sol = _cpₛₒₗ(Td[i,j], xi_d[i,j], LiCl())
            
            Td[i, j] = ( (u_d[j]*rho_d*cp_sol/dx)*Tin_x + (λ_H2O(Td[i,j])/dyd^2)*(Tn + Ts) ) / 
                       ( (u_d[j]*rho_d*cp_sol/dx) + 2*λ_H2O(Td[i,j])/dyd^2 )
        end
    end
    # ... convergencia ...
...
# --- 4. BALANCE FINAL ---
# Usamos la masa real integrada de los perfiles
m_a_dry = rho_a * sum(u_a) * dya
Wa_out = sum(Wa[Nx, :] .* u_a) / sum(u_a)
Ta_out = sum(Ta[Nx, :] .* u_a) / sum(u_a)

dm_air = m_a_dry * (W_air_in - Wa_out)
...
println("Q_air: ", Q_air)
println("Q_liq: ", Q_liq)
println("Error Masa: ", abs(dm_air - dm_liq)/dm_air * 100, " %")
println("Error Energia: ", abs(Q_air - Q_liq)/Q_air * 100, " %")
