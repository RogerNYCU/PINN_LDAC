...
#Q Cedido (Hot):   482550.00 W/m Q Ganado (Cold):  516060.64 W/m Error Balance:        6.9445 %
...
res = solve(prob, OptimizationOptimisers.Adam(0.001); maxiters = 500, 
            callback = (p, l) -> (l < 1e-4 ? true : false) || (println("Loss: $l"); false))

println(">>> Fase 2: LBFGS (Refinamiento de Balance)")
prob2 = OptimizationProblem(optf, res.u)
res = solve(prob2, LBFGS(linesearch = BackTracking()); maxiters = 200)
