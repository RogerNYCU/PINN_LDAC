#este codigo no tiene gradientes pronunciados pero tiene 11% de error
...
    return (x_v, eta_v) -> begin
        ...
        s = 1.0 / (1.0 + exp(-40.0 * eta_v)) #cambiar a 40
...
# En lugar de QuasiRandom uniforme, usemos más puntos cerca de la interfaz
# Puedes definir un vector de puntos manual o usar una estrategia de pesos
Random.seed!(42)
strategy = NeuralPDE.QuasiRandomTraining(10000) # Sube a 10k puntos
...
    # Multiplicamos la pérdida de la PDE por un factor de importancia (ej. 10.0)
    # para que a la red le "duela" más no cumplir la ecuación de calor
    l_pde = sum(f -> mean(abs2, f(θ)), pde_loss_functions) 
    #l_bc = sum(f -> mean(abs2, f(θ)), bc_loss_functions)
    return l_pde *100 #+ l_bc
...
@time res = solve(prob, OptimizationOptimisers.Adam(0.005); maxiters = 1000, callback = callback)
...
function post_procesado_y_balance(T_f)
    ...
    p1 = heatmap(xs .* Lx, y_phys_axis, T_map, cmap=:thermal,
